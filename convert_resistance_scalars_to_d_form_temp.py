...
# convert_resistance_scalars_to_d_form.py
...
#   scalars_general_resistance_blob_d.txt.
...
for lam in lam_range_with_reciprocals:
    ...
    for s_dash in s_dash_range:
        ...
        """
        Temp change 23-Jan-2019: Minus signs put on YB, XG, YG, because these have
        the wrong sign in XYZ_raw (from the Mathematica file). If I change the Mathematica
        file to fix this, then I can just use the normal file. This is primarily a
        test to see that if I change this, then do my numbers match those from the
        Brady file?
        """
...
# computer readable
with open('find_resistance_scalars/scalars_general_resistance_blob_d.txt', 'wb') as outputfile:
...
with open('find_resistance_scalars/scalars_general_resistance_text_d.txt', 'a') as outputfile:
